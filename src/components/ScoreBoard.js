...
const ScoreBoard = () => {
  ...
  const handleBye = () => {
    const runs = parseInt(prompt('Enter bye runs'));
    if (!isNaN(runs)) {
      setTotalRuns(totalRuns + runs);
      setExtras((state) => ({ ...state, total: state.total + runs }));
      setCurrentRunStack((state) => [...state, `${runs}b`]);
      setBallCount(ballCount + 1);
    }
  };

  const handleLegBye = () => {
    const runs = parseInt(prompt('Enter leg bye runs'));
    if (!isNaN(runs)) {
      setTotalRuns(totalRuns + runs);
      setExtras((state) => ({ ...state, total: state.total + runs }));
      setCurrentRunStack((state) => [...state, `${runs}lb`]);
      setBallCount(ballCount + 1);
    }
  };
  ...
  return (
    <div className='container'>
      ...
      <div className='score-container'>
        ...
        <div className='score'>
          <div>
            {inningNo % 2 !== 0 ? scoringTeam : chasingTeam} : {totalRuns}/{wicketCount} ({totalOvers})
          </div>
